...
| Approval gates for mutating actions | `README.md`, Step 4 | `src/libs/common/shell_policy.py`, coordinator/executor approval flow tests | Add private-beta guardrails and duplicate-safe notifications | Yes | PR-04 |
...
| Tenant controls for small private beta (3-5 users) | Plan default + Phase 2 prep | Current admin API has default-tenant invite only | Add tenant APIs, limits, health views | Yes | PR-06 |
| Token budget alerts/soft guardrails | Private beta ops need | Token usage table + summary endpoints exist | Add configurable per-tenant limits and graceful refusal/audit | Yes | PR-06 |
